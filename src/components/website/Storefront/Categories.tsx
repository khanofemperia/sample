...
export default function Categories({
  ...
  const resetCarousel = (screenWidth: number) => {
    ...
    const totalCategoriesWidth =
      categories.length * (categoryWidth + GAP_BETWEEN_CATEGORIES) - x;
    setIsNextButtonHidden(totalCategoriesWidth <= screenWidth);

    console.log(totalCategoriesWidth <= screenWidth);
    console.log(totalCategoriesWidth);
  ...
  return (
    <div className="bg-lime-200 relative mb-10 w-full max-w-[768px] lg:max-w-[828px] mx-auto flex justify-center">
      <div className="overflow-hidden" ref={carouselRef}>
        <div
          style={{ transform: `translateX(${distance}px)` }}
          className={clsx("w-max flex gap-5 p-1", {
